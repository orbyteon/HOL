...
- Intermediate automatic PlayMode run `34691967357` tested `2b894eb…` through merge revision `c7a174e…`: **115 passed / 1 failed / 10 explicit capture skips**. Its remaining PLAY title failure mixed a virtual portrait viewport with the batch host's real landscape RectTransform. The follow-up tests against independently calculated portrait bounds with the original **28-unit inset**; the directly affected local batch-mode test passed **1/1**. Final automatic evidence is reported on PR #90, not inferred from these earlier results.
