...
public sealed class PanelPlayCapturePlayModeTests
{
    ...
    [TestCase(true, true, "panelplay", true)]
    [TestCase(false, true, "panelplay", false)]
    [TestCase(true, false, "panelplay", false)]
    ...
    public void CaptureRequiresAndroidDevelopmentPanelPlayIntent(
        bool android, bool development, string requestedScreen, bool expected)
    {
        var method = RuntimeType("PanelPlayCaptureBootstrap").GetMethod(
    ...
    public IEnumerator NoCaptureIntentLeavesMainMenuHierarchyUntouched()
    {
        ...
        Assert.That(FindInScene(scene, RuntimeType("PanelPlayCaptureBootstrap")), Is.Null);
    ...
    public IEnumerator CaptureIteratorHasTwoBarriersAndLogsExactlyOnceWhenAdvanced()
    {
        ...
        try
        {
            ...
            var bootstraps = ComponentsInScene(
                scene, RuntimeType("PanelPlayCaptureBootstrap"));
            ...
            Assert.That((bool)markerLogged.GetValue(null), Is.False);
            LogAssert.Expect(LogType.Log, "HOL_PANELPLAY_CAPTURE_READY");
    ...
    {
        var property = RuntimeType("PanelPlayCaptureBootstrap").GetProperty(
    ...
    {
        var type = RuntimeTypeOrNull("PanelPlayCaptureBootstrap");
        if (type == null) return;
    ...
    static void InvokePlayInstaller()
    {
        var install = RuntimeType("MainMenuPlayVisuals").GetMethod(
            "Install", StaticFlags);
        Assert.That(install, Is.Not.Null);
        install.Invoke(null, null);
    }
