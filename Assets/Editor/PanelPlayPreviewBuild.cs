...
public static class PanelPlayPreviewBuild
{
    const string OutputPath = "build/Android/HOL-panelplay-debug.apk";

    public static void Build()
    {
        if (EditorUserBuildSettings.activeBuildTarget != BuildTarget.Android)
            throw new BuildFailedException(
                "PanelPlay preview builds require the Android editor target.");
        ...
        try
        {
            ...
            if (report.summary.result != BuildResult.Succeeded)
                throw new BuildFailedException(
                    "PanelPlay preview APK build failed: " + report.summary.result);
        }
        finally
        {
