...
public sealed class DailyHuntVisuals : MonoBehaviour
{
    ...
    void Build(Transform panel)
    {
        ...
        IsReady = ArtReady(
            ...
            chevron) && displayFont != null && bodyFont != null;

        if (!IsReady)
        {
            Debug.LogError(
                "[DailyHuntVisuals] Required production artwork/fonts are missing.");
            return;
        }
        ...
        rewardHeading = EnsureText(
            rewardCard.transform, "DailyRewardHeading", 36f, displayFont,
        ...
        Sprite chevron)
    {
        ...
        chipName = EnsureText(
            playerChip.transform, "DailyPlayerName", 29f, bodyFont,
            NearWhite, TextAlignmentOptions.Center);
        Place(
        ...
        chipWins = EnsureText(
            playerChip.transform, "DailyPlayerWins", 28f, bodyFont,
    ...
    void RefreshCopy()
    {
        ...
        {
            challengeHeading.text = greek
                ? "ΣΗΜΕΡΙΝΟ ΚΥΝΗΓΙ ΑΡΙΘΜΟΥ"
                : "TODAY'S NUMBER HUNT";
        }
        if (rewardHeading != null)
        {
            rewardHeading.text = greek
                ? "ΗΜΕΡΗΣΙΑ ΠΡΟΟΔΟΣ"
                : "DAILY PROGRESS";
        }
        RefreshPlayerChip();
    ...
    static void StyleInput(TMP_InputField input, Sprite frame)
    {
