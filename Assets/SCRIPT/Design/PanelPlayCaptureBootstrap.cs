...
// Development-build QA seam for deterministic Android PanelPlay screenshots.
// Normal builds never inspect Android intent state or add this component.
public sealed class PanelPlayCaptureBootstrap : MonoBehaviour
{
    const string CaptureExtra = "hol_capture_screen";
    const string PlayScreen = "panelplay";
    const string ReadyMarker = "HOL_PANELPLAY_CAPTURE_READY";
        ...
        bool android, bool development, string requestedScreen)
    {
        return android &&
               development &&
               string.Equals(requestedScreen, PlayScreen, StringComparison.Ordinal);
    }
    ...
    static void InstallForScene(Scene scene, LoadSceneMode mode)
    {
        ...
        {
            if (root.GetComponentInChildren<PanelPlayCaptureBootstrap>(true) != null)
            {
                SceneManager.sceneLoaded -= InstallForScene;
                return;
            }
        ...
        if (host == null && roots.Length > 0)
            host = roots[0];
        if (host != null)
            host.AddComponent<PanelPlayCaptureBootstrap>();
    ...
    void Update()
    {
        ...
        if (!openedPlay)
        {
            ...
            if (menu == null) return;
            // Capture the same production route a player uses. The seam never
            // forces selector hierarchy state directly.
            menu.OnPlayPressed();
            openedPlay = true;
            return;
        }
        ...
        if (playVisuals == null || !playVisuals.IsReady || !playVisuals.IsSettled)
            return;
