...
export const validatePanelPlayPng = png => {
  ...
  requireCondition(
    colors.size > 1,
    "PanelPlay screenshot is uniform: sampled only one RGB color");
  requireCondition(
    luminanceRange >= minimumLuminanceRange,
    `PanelPlay screenshot luminance range ${luminanceRange.toFixed(2)} is too small`);
  requireCondition(
    channelRange >= minimumChannelRange,
    `PanelPlay screenshot color range ${channelRange} is too small`);
...
if (isMain) {
  try {
    ...
    requireCondition(path, "Usage: validate-panelplay-screenshot.mjs <panelplay.png>");
    const result = validatePanelPlayPng(fs.readFileSync(path));
    console.log(
      `Validated ${result.width}x${result.height} PanelPlay PNG: ` +
      `${result.sampledColors} sampled colors, ` +
      `${result.luminanceRange.toFixed(2)} luminance range, ` +
      `${result.channelRange} channel range`);
  } catch (error) {
    console.error("PanelPlay screenshot validation failed: " + error.message);
    process.exitCode = 1;
  }
}
